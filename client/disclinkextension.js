(function (Scratch) {
  ...
  let defaultIdentity = { username: '', avatarURL: '' };
  ...
  function connectWS(url){
    ...
    ws.addEventListener('message', (evt) => {
      ...
      if (payload?.type === 'messageCreate' && payload.data) {
        ...
        // store fetched bulk messages
        lastMessage.bulkMessages = payload.data;
      } else if(payload?.type === 'userProfile' && payload.ok && payload.data){
        defaultIdentity.username = payload.data.username;
        defaultIdentity.avatarURL = payload.data.avatarURL;
      }
    });
  }
  ...
  class DiscordBridge {
    getInfo() {
      return {
        ...
        blocks: [
          ...
          // Impersonation
          { opcode: 'impersonateUser', blockType: BlockType.COMMAND, text:'impersonate user id [USER]', arguments:{ USER:{ type: ArgumentType.STRING, defaultValue:'' } } },
          { opcode: 'clearIdentity', blockType: BlockType.COMMAND, text:'clear impersonation' }
        ]
      };
    }
    ...
    // Bulk messages
    getMessages(args){
      return lastMessage.bulkMessages || ''; // Already returned in ␟ / §§§ format
    }

    // Impersonation
    impersonateUser(args){
      if(!this.isConnected()) return;
      const userId = String(args.USER || ''); if(!userId) return;
      const ref = Math.random().toString(36).slice(2);
      const onMsg = (evt)=>{
        let p; try{p=JSON.parse(evt.data);}catch{return;}
        if(p?.type==='userProfile' && p.ref===ref && p.ok){
          defaultIdentity.username = p.data.username || '';
          defaultIdentity.avatarURL = p.data.avatarURL || '';
          ws.removeEventListener('message', onMsg);
        }
      };
      ws.addEventListener('message', onMsg);
      ws.send(safeJSON({ type:'getUserProfile', ref, userId }));
    }

    clearIdentity(){ defaultIdentity={ username:'', avatarURL:'' }; }
  }
