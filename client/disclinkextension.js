(function (Scratch) {
  ...
  const messageQueue = [];
  let lastMessage = { content:'', author:'', channelId:'', channelName:'', channelType:null, guildId:'', bot:false };
  let guildChannels = {};
  ...
  function normalizeName(str) { return String(str ?? '').trim().replace(/\s+/g,' ').toLowerCase(); }
  ...
  function connectWS(url) {
    ...
    ws.addEventListener('open', () => { connected = true; ws.send(safeJSON({ type:'getGuildChannels' })); });
    ws.addEventListener('close', () => { connected = false; setTimeout(() => connectWS(wsUrl),1500); });
    ws.addEventListener('message', (evt) => {
      let payload;
      try { payload = JSON.parse(evt.data); } catch { return; }
      if (payload?.type === 'messageCreate' && payload.data) {
        const d = payload.data;
        lastMessage = { content: String(d.content ?? ''), author: String(d.author?.username ?? ''), channelId: String(d.channelId ?? ''), channelName: String(d.channelName ?? ''), channelType: d.channelType ?? null, guildId: String(d.guildId ?? ''), bot: !!d.author?.bot };
  ...
  class DiscordBridge {
    ...
    sendMessage(args) { if (!this.isConnected()) return; ws.send(safeJSON({ type:'sendMessage', ref:Math.random().toString(36).slice(2), channelId:String(args.CHANNEL ?? ''), content:String(args.TEXT ?? '') })); }

    sendMessageByName(args) {
      if (!this.isConnected()) return;
      const guild = Object.values(guildChannels).find(g => normalizeName(g.guildName) === normalizeName(args.SERVER));
      if (!guild) return;
      const channel = guild.channels.find(c => normalizeName(c.name) === normalizeName(args.CHANNEL));
      if (!channel) return;
    ...
    textChannels(args) { const g = guildChannels[args.GUILD]; if(!g) return ''; return g.channels.filter(c=>c.type===0).map(c=>c.name).join(', '); }
    voiceChannels(args) { const g = guildChannels[args.GUILD]; if(!g) return ''; return g.channels.filter(c=>c.type===2).map(c=>c.name).join(', '); }
    channelsWithTypes(args) { const g = guildChannels[args.GUILD]; if(!g) return ''; return g.channels.map(c=>`${c.name} (${c.type})`).join(', '); }
